...
class Colors(Enum):
    ...
    def startswith(self, letter):
        if str(self)[0] == letter:
            return True
        return False
    
    def lower(self):
        colorStr = str(self).lower()
        #remove colors. from the start
    ...
    def color_to_rgba(color: Color | str, alpha: float = 1) -> np.ndarray:
        return np.array([*Colors.color_to_rgb(color), alpha])
    ...
    def rgba_to_color(rgba: Iterable[float]) -> Color:
        return Colors.rgb_to_color(rgba[:3])
    ...
    def invert_color(color: Color) -> Color:
        return Colors.rgb_to_color(1.0 - Colors.color_to_rgb(color))

    @staticmethod
    def color_to_int_rgb(color: Color) -> np.ndarray:
        return (255 * Colors.color_to_rgb(color)).astype("uint8")

    @staticmethod
    def color_to_int_rgba(color: Color, opacity: float = 1.0) -> np.ndarray:
        alpha_multiplier = np.vectorize(lambda x: int(x * opacity))

        return alpha_multiplier(np.append(Colors.color_to_int_rgb(color), 255))
    ...
    def color_gradient(
        ...
        rgbs = list(map(Colors.color_to_rgb, reference_colors))
        ...
        floors[-1] = len(rgbs) - 2
        return [
            Colors.rgb_to_color(interpolate(rgbs[i], rgbs[i + 1], alpha))
            for i, alpha in zip(floors, alphas_mod1)
        ]

    @staticmethod
    def interpolate_color(color1: Color, color2: Color, alpha: float) -> Color:
        rgb = interpolate(Colors.color_to_rgb(color1), Colors.color_to_rgb(color2), alpha)
        return Colors.rgb_to_color(rgb)

    @staticmethod
    def average_color(*colors: Color) -> Color:
        rgbs = np.array(list(map(Colors.color_to_rgb, colors)))
        mean_rgb = np.apply_along_axis(np.mean, 0, rgbs)
        return Colors.rgb_to_color(mean_rgb)

    @staticmethod
    def random_bright_color() -> Color:
        color = Colors.random_color()
        curr_rgb = Colors.color_to_rgb(color)
